package cat.politecnicllevant.prova.middleware;

import cat.politecnicllevant.prova.service.JwtPrincipal;
import cat.politecnicllevant.prova.service.JwtService;
...
/**
 * Filtre encarregat de:
 *  - Llegir el JWT de la capçalera Authorization (Bearer token)
 *  - Validar el token
 *  - Extreure l'usuari i els rols
 *  - Crear un Authentication i inserir-lo al SecurityContext
 *
 * Aquest filtre permet utilitzar Spring Security com a motor
 * d'autorització (@PreAuthorize) sense UserDetailsService.
 */
@Component
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        // GET /api/movie és públic i NO volem ni mirar si hi ha token
        if ("/api/movie".equals(path) && "GET".equalsIgnoreCase(method)) return true;
