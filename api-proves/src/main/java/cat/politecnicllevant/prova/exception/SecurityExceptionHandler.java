...
public class SecurityExceptionHandler implements AuthenticationEntryPoint, AccessDeniedHandler {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException)
        ...
        // Resposta quan l'usuari no està autenticat (401).
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException accessDeniedException)
            throws IOException, ServletException {
        // Resposta quan l'usuari està autenticat però no té permisos (403).
    ...
    private void writeProblemDetail(HttpServletResponse response, HttpStatus status, String detail, String path)
            throws IOException {
        // Cos d'error estàndard en format ProblemDetail.
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(status, detail);
        pd.setProperty("timestamp", Instant.now());
        pd.setProperty("path", path);

        // Escriu la resposta com a JSON.
